...
function chainPromises(array, action) {
  ...
  const chain = array.reduce(async (q1, q2) => {
    try {
      const prevRes = await q1;
      const nextRes = await q2;
      if (prevRes && nextRes) {
        return action(prevRes, nextRes);
      }
      return q1;
    } catch (error) {
      return q1;
  ...
  return chain.then((value) => value).catch((error) => error);
}
